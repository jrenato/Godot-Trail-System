...
var _target: Node3D
var _wire_mesh_instance: MeshInstance3D = MeshInstance3D.new()
	...
	if alignment == "View":
		if get_viewport().get_camera_3d():
			...
			var path_direction: Vector3 = (
				(point.transform.origin - point_prev.transform.origin).normalized()
			)
			normal = (
				(cam_pos - (point.transform.origin + point_prev.transform.origin) / 2)
...
func _render_geometry(source: Array) -> void:
	...
	for i in range(1, points_count):
		...
		var vertices = _prepare_geometry(
			to_be_rendered[i - 1], to_be_rendered[i], half_width, 1.0 - factor
		)
		if tiled_texture:
			if tiling > 0:
				factor *= tiling
				...
				var travel = (
					(to_be_rendered[i - 1].transform.origin - to_be_rendered[i].transform.origin)
					. length()
				)
				u += travel / base_width
				factor = u

		_immediate_mesh.surface_set_color(_color)
		_immediate_mesh.surface_set_uv(Vector2(factor, 0))
		_immediate_mesh.surface_add_vertex(vertices[0])
		_immediate_mesh.surface_set_uv(Vector2(factor, 1))
		_immediate_mesh.surface_add_vertex(vertices[1])
	...
	if show_wireframe:
		...
		_wire_obj.clear_surfaces()
		_wire_obj.surface_begin(Mesh.PRIMITIVE_LINE_STRIP, null)
		_wire_obj.surface_set_color(wireframe_color)
		_wire_obj.surface_set_uv(Vector2(0.5, 0.5))
		for i in range(1, wire_points.size() - 2, 2):
			## order: i-1, i+1, i, i+2
			_wire_obj.surface_add_vertex(wire_points[i - 1])
			_wire_obj.surface_add_vertex(wire_points[i + 1])
			_wire_obj.surface_add_vertex(wire_points[i])
			_wire_obj.surface_add_vertex(wire_points[i + 2])
		_wire_obj.surface_end()
...
func _chaikin(A, B, C) -> Array:
	...
	if smoothing_iterations == 1:
		...
		if smoothing_iterations == 2:
			out += [
				Point.new(A2_t, A2_a),
				Point.new(A11_t, A11_a),
				Point.new(B11_t, B11_a),
				Point.new(B2_t, B2_a)
			]
			...
			out += [
				Point.new(A2_t, A2_a),
				Point.new(A121_t, A121_a),
				Point.new(A12_t, A12_a),
				Point.new(B12_t, B12_a),
				Point.new(B121_t, B121_a),
				Point.new(B2_t, B2_a)
			]
...
func _ready() -> void:
	...
	# _wire_obj.surface_set_material(0, _wire_mat)
	# _wire_obj.material_override = _wire_mat
	_wire_mesh_instance.mesh = _wire_obj
	add_child(_wire_mesh_instance)
